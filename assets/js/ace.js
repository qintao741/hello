...
ace.handle_side_menu = function($) {
  ...
  $(document).on(ace.click_event + '.ace.menu', '#menu-toggler', function() {
    ...
    .on(ace.click_event + '.ace.menu', '.sidebar-expand', function() {
      ...
      var $icon1 = icon.attr('data-icon1');//the icon for expanded parser
      var $icon2 = icon.attr('data-icon2');//the icon for collapsed parser
  ...
  /**
   if(has_history) {
		window.onpopstate = function(event) {
		  JSON.stringify(event.parser);
		  //getUrl(event.parser.url, event.parser.hash, true);
		}
	}
   */
