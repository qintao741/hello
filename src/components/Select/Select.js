...
var Select = React.createClass({
    ...
    propTypes: {
        ...
        disableFilterOptions: React.PropTypes.bool,      // 不使用 filter options
        ...
        matchProp: React.PropTypes.string,         // (any|text|value) which option property to filter on
        multi: React.PropTypes.bool,               // multi-value input
        name: React.PropTypes.string,              // field name, for hidden <input /> tag
        addLabelText: React.PropTypes.string,      // placeholder displayed when you want to add a text on a multi-value input
        noResultsText: React.PropTypes.string,     // placeholder displayed when there are no matching search results
        onBlur: React.PropTypes.func,              // onBlur handler: function(event) {}
        onChange: React.PropTypes.func,            // onChange handler: function(newValue) {}
        onFocus: React.PropTypes.func,             // onFocus handler: function(event) {}
        onOptionLabelClick: React.PropTypes.func,  // onCLick handler for value texts: function (value, event) {}
        optionRenderer: React.PropTypes.func,      // optionRenderer: function(option) {}
        options: React.PropTypes.array,            // array of options
        placeholder: React.PropTypes.string,       // field placeholder, displayed when there's no value
        searchable: React.PropTypes.bool,          // whether to enable searching feature or not
        searchPromptText: React.PropTypes.string,  // text to prompt for search input
    ...
    getStateFromValue: function (value, options, placeholder) {
        ...
        return {
            ...
            placeholder: !this.props.multi && values.length ? values[0].text : placeholder,
    ...
    handleMouseDown: function (event) {
        ...
        if (this.props.disabled || (event.type === 'mousedown' && event.button !== 0)) {
            if (this.props.onClickWhenDisable) {
                this.props.onClickWhenDisable();
            }
            return;
        }
    ...
    filterOptions: function (options, values) {
        if (!this.props.searchable || this.props.disableFilterOptions) {
            return options || [];
        }
        ...
        if (this.props.filterOptions) {
            ...
            var filterOption = function (op) {
                ...
                var valueTest = String(op.value), textTest = String(op.text);
                ...
                (this.props.matchProp !== 'value' && textTest.substr(0, filterValue.length) === filterValue)
                ) : (
                (this.props.matchProp !== 'text' && valueTest.indexOf(filterValue) >= 0) ||
                (this.props.matchProp !== 'value' && textTest.indexOf(filterValue) >= 0)
                );
            };
    ...
    render: function () {
        ...
        if (!this.state.inputValue && (!this.props.multi || !value.length)) {
            value.push(<div
                data-tip={this.props.hasError ? this.props.errorTip : undefined}
                data-type="error"
                className={classes("Select-placeholder", {"has-error": this.props.hasError})}
                key="placeholder">{this.state.placeholder}</div>);
        }

        var loading = this.state.isLoading ? <span className="Select-loading" aria-hidden="true"/> : null;
        var clear = this.props.clearable && this.state.value && !this.props.disabled ?
            <span className="Select-clear"
                  title={this.props.multi ? this.props.clearAllText : this.props.clearValueText}
                  aria-text={this.props.multi ? this.props.clearAllText : this.props.clearValueText}
                  onMouseDown={this.clearValue} onClick={this.clearValue}
